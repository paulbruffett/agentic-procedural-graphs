...
- Graph-evolution tracking and the `analysis/` folder (2026-09-19, on request). Core: `ProceduralGraph.diff()` -> `GraphDiff` (added / removed / revised nodes and edges, order-independent; it replaced evolve's `_canonical` no-op check); `evolve` saves every round's `EditSet` to `edits/round_k.json`, accepted or not (before this a rejected proposal survived only as a one-line summary), and logs a cumulative `edge_changes` table to W&B. Everything that renders lives in top-level `analysis/`, which may import `pg` but is never imported by it: `graph_evolution.py` (`diff a b`; `report <run dir>` -> `evolution.md` with a Mermaid diagram + text diff per round, and `timeline.html`, a self-contained interactive page laid out once with dagre so nodes stay put across rounds), plus the code walkthrough (`analysis/walkthrough.md`, which opens with a conceptual map: roles, how a graph is developed and maintained, and how the refiner is evaluated). Same day: the paired statistics moved too, from `src/pg/analyze.py` / `pg analyze` to `analysis/paired_comparison.py` (`uv run python -m analysis.paired_comparison <eval run dirs...>`, same options); `pg` now has only gen-data | evolve | eval. Older notes below that say `pg analyze` predate this.
